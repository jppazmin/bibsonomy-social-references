...
public class SimHash {
	 ...
	 * 
	 */
	public static String getSimHash(final Resource resource,
			final HashID simHash) {
	...
	public static String getSimHash(final BibTex bibtex, final HashID simHash) {
		if (simHash.getId() == HashID.SIM_HASH0.getId()) {
			...
			throw new RuntimeException("SimHash " + simHash.getId()
					+ " doesn't exist.");
		}
	}
	 ...
	 * @return the corresponding simhash for a bookmark.
	 */
	public static String getSimHash(final Bookmark bookmark,
			final HashID simHash) {
		if (simHash.getId() == HashID.SIM_HASH0.getId()) {
	...
	/**
	 * Calculates the simHash0 for a bookmark, which consideres: url of a
	 * bookmark Currently, all hashes for bookmark are equal.
	 * 
	 * @param bookmark
	 *            the object whose hash is to be calculated
	...
	/**
	 * Calculates the simHash1 for a bookmark, which consideres: url of a
	 * bookmark Currently, all hashes for bookmark are equal.
	 * 
	 * @param bookmark
	 *            the object whose hash is to be calculated
	...
	/**
	 * Calculates the simHash2 for a bookmark, which consideres: url of a
	 * bookmark Currently, all hashes for bookmark are equal.
	 * 
	 * @param bookmark
	 *            the object whose hash is to be calculated
	...
	/**
	 * Calculates the simHash3 for a bookmark, which consideres: url of a
	 * bookmark Currently, all hashes for bookmark are equal.
	 * 
	 * @param bookmark
	 *            the object whose hash is to be calculated
	 ...
	 * @param bibtex
	 *            the object whose hash is to be calculated
	 * @return the calculated simHash0, which consideres: title, author, editor,
	 *         year, entrytype, journal, booktitle.
	 */
	public static String getSimHash0(final BibTex bibtex) {
		return StringUtils.getMD5Hash(StringUtils
				.removeNonNumbersOrLettersOrDotsOrSpace(bibtex.getTitle())
				+ " "
				+ StringUtils.removeNonNumbersOrLettersOrDotsOrSpace(bibtex
						.getAuthor())
				+ " "
				+ StringUtils.removeNonNumbersOrLettersOrDotsOrSpace(bibtex
						.getEditor())
	...
	public static String getSimHash1(final BibTex bibtex) {
		if (StringUtils.removeNonNumbersOrLetters(bibtex.getAuthor())
				.equals("")) {
	 ...
	 * 
	 * - entsprechende zusätzliche Spalten (Volume/Number) im ResourceHandler
	 * mit herausgeben (in getBibtexSelect() columns[] anpassen) - Bibtex.java
	 * anpassen: getSimHash(), setHashesToNull(), später getHash() - writing of
	 * hashes in insertBibIntoDB() in BibtexHandler is already done for 0 to 4 -
	 * update queries in ResourceHandler to use correct hashes for query
	 * 
	 * changing in Bibtex.getHash() from 0 to 2 should imply changing column in
	 * resource handler and pre-0 to pre-2 in JSPs?!
	 * 
	 * have a look at SIM_HASH and INTRA_HASH - where are they used and should
	 * they be changed?
	 */
	/**
	 * @param bibtex
	 *            the object whose hash is to be calculated
	 * @return the calculated simHash0, which consideres: author, editor, year,
	 *         entryType, journal, booktitle, volume, number.
	 */
	...
	private static String getNormalizedAuthor(final String str) {
		if (str == null)
			return "";
		return StringUtils.getStringFromList(normalizePersonList(StringUtils
				.removeNonNumbersOrLettersOrDotsOrSpace(str.toLowerCase())));
	}
